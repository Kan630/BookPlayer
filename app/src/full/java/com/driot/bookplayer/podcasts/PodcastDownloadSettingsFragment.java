...
/**
 * Per-podcast download settings, reached from PodcastEpisodeFragment. Converted from
 * the former PodcastDownloadSettingsActivity - see [[radio_deeplink_applinks_fix]].
 */
@AndroidEntryPoint
public class PodcastDownloadSettingsFragment extends LoggingFragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        return inflater.inflate(R.layout.activity_podcast_download_settings, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        tvPodcastName = view.findViewById(R.id.tvDownloadSettingsPodcastName);
        tvTotalStorageValue = view.findViewById(R.id.tvTotalStorageValue);
        tvDownloadLastN = view.findViewById(R.id.tvDownloadLastN);
        tvDownloadLastNSize = view.findViewById(R.id.tvDownloadLastNSize);
        ivPodcastCover = view.findViewById(R.id.ivDownloadSettingsPodcastCover);
        rowAutoDownload = view.findViewById(R.id.rowAutoDownload);
        seekbarDownloadLastN = view.findViewById(R.id.seekbarDownloadLastN);
        etDownloadLastN = view.findViewById(R.id.etDownloadLastN);
        btnDownloadLastN = view.findViewById(R.id.btnDownloadLastN);
        tvStatusNeverPlayedCount = view.findViewById(R.id.tvStatusNeverPlayedCount);
        tvStatusPlayedCount = view.findViewById(R.id.tvStatusPlayedCount);
        tvStatusPendingAutoDeleteCount = view.findViewById(R.id.tvStatusPendingAutoDeleteCount);
        tvStatusOrphanCount = view.findViewById(R.id.tvStatusOrphanCount);
        tvStatusNeverDownloadedCount = view.findViewById(R.id.tvStatusNeverDownloadedCount);
        tvStatusDeletedCount = view.findViewById(R.id.tvStatusDeletedCount);
        tvStatusTotal = view.findViewById(R.id.tvStatusTotal);
        rowStatusNeverPlayed = view.findViewById(R.id.rowStatusNeverPlayed);
        rowStatusPlayed = view.findViewById(R.id.rowStatusPlayed);
        rowStatusPendingAutoDelete = view.findViewById(R.id.rowStatusPendingAutoDelete);
        rowStatusOrphan = view.findViewById(R.id.rowStatusOrphan);
        rowStatusNeverDownloaded = view.findViewById(R.id.rowStatusNeverDownloaded);
        rowStatusDeleted = view.findViewById(R.id.rowStatusDeleted);
        ...
        rowAutoDownload.setOnCheckedChangeListener((buttonView, isChecked) -> {
            ...
            viewModel.setAutoDownload(requireContext(), podcast, isChecked);
        });

        viewModel.getTotalStorageBytesLive().observe(getViewLifecycleOwner(), bytes -> {
            if (bytes != null) {
                tvTotalStorageValue.setText(Tonio.getReadableSize(bytes));
            }
        });

        viewModel.getUndownloadedCountLive().observe(getViewLifecycleOwner(), count -> {
            setupDownloadLastNControl(count != null ? count : 0);
        });

        viewModel.getUndownloadedSizesPrefixSumLive().observe(getViewLifecycleOwner(), prefixSums -> {
            undownloadedSizesPrefixSum = prefixSums != null ? prefixSums : new long[0];
            setDownloadLastNValue(pendingDownloadLastN);
        });

        viewModel.getEpisodeStatusCountsLive().observe(getViewLifecycleOwner(), counts -> {
            if (counts == null)
                return;
        ...
        btnDownloadLastN.setOnClickListener(v -> {
            ...
            viewModel.downloadLastN(requireContext(), podcast, pendingDownloadLastN, () -> {
                myToast(getString(R.string.podcast_download_last_n_started, pendingDownloadLastN));
                viewModel.refreshStats(requireContext(), podcast);
