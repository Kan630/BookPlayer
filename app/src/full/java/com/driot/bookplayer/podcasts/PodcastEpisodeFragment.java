...
/**
 * Episode list / podcast detail screen, reached from GetPodcastFragment /
 * PodcastSearchResultsFragment / PodcastFavoritesFragment / the podcast mini-player.
 * Converted from the former PodcastEpisodeActivity - see [[radio_deeplink_applinks_fix]].
 */
@AndroidEntryPoint
public class PodcastEpisodeFragment extends LoggingFragment
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        // Registered after super.onViewCreated() so it fires before BaseActivity's own back
        ...
        tvTitle = view.findViewById(R.id.tvPodcastTitle);
        tvDescription = view.findViewById(R.id.tvPodcastDescription);
        tvApiWarning = view.findViewById(R.id.tvApiWarning);
        tvApiWarning.setVisibility(View.GONE);
        tvStats = view.findViewById(R.id.tvPodcastStat);
        tvToolbarStats = view.findViewById(R.id.tvToolbarStats);
        tvSearchStat = view.findViewById(R.id.tvSearchStat);

        ivCover = view.findViewById(R.id.ivPodcastCover);
        ivMiniCover = view.findViewById(R.id.ivMiniCover);

        recyclerEpisodes = view.findViewById(R.id.rvEpisodes);

        // TOOLBAR actions
        btnFavoriteToolbar = view.findViewById(R.id.btnFavoriteToolbar);
        btnAutoDownloadToolbar = view.findViewById(R.id.btnAutoDownloadToolbar);
        btnSearchToolbar = view.findViewById(R.id.btnSearchToolbar);
        btnRefreshToolbar = view.findViewById(R.id.btnRefreshToolbar);
        btnSortToolbar = view.findViewById(R.id.btnSortToolbar);
        btnFilterToolbar = view.findViewById(R.id.btnFilterToolbar);
        btnCollapseToolbar = view.findViewById(R.id.btnCollapseToolbar);

        // OVERLAY actions on top of the big cover
        btnFavoriteOverlay = view.findViewById(R.id.btnFavoriteOverlay);
        btnAutoDownloadOverlay = view.findViewById(R.id.btnAutoDownloadOverlay);
        btnSearchOverlay = view.findViewById(R.id.btnSearchOverlay);
        btnRefreshOverlay = view.findViewById(R.id.btnRefreshOverlay);
        btnSortOverlay = view.findViewById(R.id.btnSortOverlay);
        btnFilterOverlay = view.findViewById(R.id.btnFilterOverlay);
        btnCollapseOverlay = view.findViewById(R.id.btnCollapseOverlay);

        layoutSearch = view.findViewById(R.id.layoutSearch);
        layoutSearch.setVisibility(View.GONE);
        etSearch = view.findViewById(R.id.etSearch);
        cbSearchInDescription = view.findViewById(R.id.cbSearchInDescription);
        ...
        podcastDao = AppDatabase.getDatabase(requireContext()).podcastDao();

        podcast = requireArguments().getParcelable("podcast");
        ...
        podcastEpisodeViewModel.getPodcastLiveByFeedId(podcastFeed.id).observe(getViewLifecycleOwner(), updatedPodcast -> {
            if (updatedPodcast != null) {
                this.podcast = updatedPodcast;
                // Auto-download can now also be toggled from PodcastDownloadSettingsFragment,
        ...
        podcastEpisodeViewModel.getEpisodesLive().observe(getViewLifecycleOwner(), episodes -> {
            if (episodes != null) {
                updateAdapter(episodes);
            }
        });

        podcastEpisodeViewModel.getSearchQueryLive().observe(getViewLifecycleOwner(), q -> {
            if (!etSearch.getText().toString().equals(q)) {
                etSearch.setText(q);
            }
        });

        podcastEpisodeViewModel.getSearchInDescriptionLive().observe(getViewLifecycleOwner(), inDesc -> {
            if (cbSearchInDescription.isChecked() != inDesc) {
                cbSearchInDescription.setChecked(inDesc);
            }
        });

        podcastEpisodeViewModel.getSortNewestFirstLive().observe(getViewLifecycleOwner(), sort -> {
            this.sortNewestFirst = sort;
        });

        podcastEpisodeViewModel.getShowOnlyNeverDownloadedLive().observe(getViewLifecycleOwner(), showOnlyNeverDownloaded -> {
            updateFilterIconColor(Boolean.TRUE.equals(showOnlyNeverDownloaded));
            filterAndUpdateList();
        });

        podcastEpisodeViewModel.getIsExpandedLive().observe(getViewLifecycleOwner(), expanded -> {
        ...
        podcastEpisodeViewModel.getCurrentEpisodeLive().observe(getViewLifecycleOwner(), ep -> {
            this.currentEpisode = ep;
        });

        podcastEpisodeViewModel.getToastMessageLive().observe(getViewLifecycleOwner(), msg -> {
            if (msg != null) {
                myToast(msg);
            }
        });

        podcastEpisodeViewModel.getApiErrorLive().observe(getViewLifecycleOwner(), error -> {
            if (error != null) {
        ...
        ivMiniCover.setOnClickListener(v -> {
        ...
        if (podcastFeed.id == -1) {
            ...
            if (podcastEpisodeViewModel.getEpisodesLive().getValue() == null) {
                podcastEpisodeViewModel.fetchEpisodes(requireContext(), podcast, false);
            }
        }

        View.OnClickListener favoriteClick = v -> toggleFavorite();
        View.OnClickListener autoDownloadClick = v -> {
            Bundle args = new Bundle();
            args.putParcelable("podcast", podcast);
            Navigation.findNavController(rootView).navigate(R.id.podcastDownloadSettingsFragment, args);
        };
        ...
        View.OnClickListener refreshClick = v -> {
            ...
            podcastEpisodeViewModel.fetchEpisodes(requireContext(), podcast, true);
        };
    ...
    private void toggleSearch() {
        ...
        View collapsingHeader = rootView.findViewById(R.id.collapsing);
        ...
        if (layoutSearch.getVisibility() == View.VISIBLE) {
            ...
            etSearch.setText("");
            ViewHelper.hideKeyboard(requireActivity(), etSearch);
            filterAndUpdateList();
            ...
            ViewHelper.showKeyboard(requireActivity(), etSearch);
        }
    }
    ...
    private void playEpisode(DisplayableEpisode ep) {
        if (ep == null)
            return;
        boolean online = NetworkHelper.isConnected(requireContext());
        if (!online) {
            myToast(getString(R.string.no_internet_connection));
            return;
        }
        PodcastHelper.onPodcastClick(requireContext().getApplicationContext(), ep, podcast,
                "PodcastEpisodeFragment - adapter callback: .onPlayEpisode()");
    ...
    public void onOpenLocalEpisode(ZikFile zikFile) {
        StartPlayHelper.onZikFileFromPodcast(requireContext(), zikFile, this.getClass().getSimpleName() + ".onOpenLocalEpisode()",
                sortNewestFirst);
    }
    ...
    private void proceedWithDownload(String futureFolderName, DisplayableEpisode displayableEpisode, long feedId) {
        File targetFolder = PodcastHelper.buildPodcastPath(requireContext(), futureFolderName);
