...
/**
 * The full player screen (cover/visualizer or TTS text, speed, sleep, seek bar and transport
 * buttons), extracted from PlayActivity so it can also live in a pane (two-pane book screen on
 * tablets). PlayActivity is now a thin host: window insets, orientation lock, back handling and
 * the media controller binding stay there.
 *
 * Whenever the player should go away (playlist finished, auto-sleep, fatal error, nothing to
 * play) it calls {@link #closePlayer()}, which asks the parent fragment or the activity if one
 * of them implements {@link Host}, and finishes the activity otherwise.
 */
@AndroidEntryPoint
public class PlayerFragment extends LoggingFragment implements TtsOverlayManager.Host {

    /** Implemented by whatever hosts the player (PlayActivity finishes itself). */
    public interface Host {
        void onPlayerCloseRequested();
    }
    ...
    private final Runnable dragResetRunnable = new Runnable() {
        @Override
        public void run() {
            PlaybackCommands.resetLastUserAction(appContext);
    ...
    // --- Broadcasts we still care about at the player level (UI only) ---
    private final BroadcastReceiver uiReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context c, Intent i) {
            ...
            if (MediaService.NOTIFICATION_ERROR.equals(action)) {
                ...
                myLog("PlayerFragment: closing due to ACTION_FINISH_PLAYER_ACTIVITIES");
                closePlayer();
            }
        }
    };
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        messageOverlay = view.findViewById(R.id.message_overlay);

        bPlayPause = view.findViewById(R.id.ibPlayPause);
        bRewind = view.findViewById(R.id.mbRewind);
        bForward = view.findViewById(R.id.mbForward);
        bSpeedUp = view.findViewById(R.id.bSpeedUp);
        bSpeedDown = view.findViewById(R.id.bSpeedDown);
        bSetSleep = view.findViewById(R.id.bSetSleep);

        tvCurTime = view.findViewById(R.id.textViewSeekBar);
        tvTotalTime = view.findViewById(R.id.textViewTempsTotal);
        ...
        tvListeningTime = view.findViewById(R.id.tv_ListeningTime);
        tvTimeLeft = view.findViewById(R.id.tv_TimeLeft);

        ttsContainer = view.findViewById(R.id.ttsContainer);
        rvTtsText = view.findViewById(R.id.rvTtsText);

        ClickInterceptFrameLayout coverContainerClickIntercept = view.findViewById(R.id.coverContainer);

        sbSeek = view.findViewById(R.id.sbSeek);
        heatMapSeek = view.findViewById(R.id.heatMapSeek);
        ivCover = view.findViewById(R.id.folderImage);
        ivCover.setImageURI(null);
        frequencyVisualizerView = view.findViewById(R.id.frequencyVisualizerView);
        ...
        if (isTextBook) {
            ...
            ttsReaderController = new TtsReaderController(requireContext(), rvTtsText);
            ttsReaderController.bind(getViewLifecycleOwner(), vm);
            ttsOverlayManager = new TtsOverlayManager(this, requireContext(), ttsManager);
            view.findViewById(R.id.btnToggleTtsView).setOnClickListener(v -> {
                TtsReaderFragment.start(requireContext());
            });
            view.findViewById(R.id.ib_tts_settings).setOnClickListener((v) -> {
        ...
        coverContainerClickIntercept.setCallbacks(new ClickInterceptFrameLayout.Callbacks() {
            ...
            public void onLongPress() {
                ...
                if (z != null)
                    MetadataUi.showMetadataDialog(requireContext(), z);
            }
        });
        ...
        if (isTextBook) {
            ...
            vm.getTtsRange().observe(getViewLifecycleOwner(), p -> {
        ...
        LocalBroadcastManager lb = LocalBroadcastManager.getInstance(requireContext());
    ...
    public void onDestroyView() {
        ...
        try {
            LocalBroadcastManager.getInstance(requireContext()).unregisterReceiver(uiReceiver);
    ...
    public void onResume() {
        super.onResume();
        screensaverActive = false; // Reset when returning to the player
    ...
    /** The player should go away: let the host decide how (PlayActivity finishes). */
    protected void closePlayer() {
        Fragment parent = getParentFragment();
        if (parent instanceof Host) {
            ((Host) parent).onPlayerCloseRequested();
        } else if (getActivity() instanceof Host) {
            ((Host) getActivity()).onPlayerCloseRequested();
        } else if (getActivity() != null) {
            getActivity().finish();
        }
    }

    /** True while the player is going away - don't start anything new (e.g. the screensaver). */
    private boolean isClosing() {
        return isRemoving() || getActivity() == null || getActivity().isFinishing();
    }

    /** runOnUiThread replacement: skipped once the view is gone. */
    private void runOnUi(Runnable r) {
        mainHandler.post(() -> {
            if (getView() != null) r.run();
        });
    }
    ...
    private void showSleepDialog() {
        ...
        View dialogView = LayoutInflater.from(requireContext()).inflate(R.layout.dialog_set_sleep, null);
    ...
    private void initTtsVoiceSpinner(Folder folder) {
        ...
        Spinner spinnerTtsVoice = requireView().findViewById(R.id.spinnerTtsVoice);
        ...
        vm.setupTtsVoiceSpinner(
                ...
                voice -> {
                    ...
                    AppDatabase.databaseWriteExecutor.execute(() -> {
                        AppDatabase.getInstance(appContext).folderDao().update(folder);
                    });
                    ...
                    try {
                        if (vm != null) {
                            ...
                            vm.warmUpTtsVoice(picked, (ready, reason) -> runOnUi(() -> {
                                spinnerTtsVoice.setEnabled(true);
    ...
    private void closeAndShowFatalError(String errMessage, String path) {
        ErrorUi.showPlayAudioErrorMessage(requireContext(), errMessage, path);
        closePlayer();
    }
    ...
    private void updateVisualizer(PlaybackUiState s) {
        ...
        if (Option.getVisualizerOn() && PermissionRequest.isRecordAudioPermissionGranted(requireContext())
                && sessionId != null) {
            try {
