...
public class FinalParseFolderWorker extends ImportWorker {
    ...
    private void addTextFileUnique(DocumentFile df) {
        ...
        audioFileInfoArrayList.add(new AudioFileInfo(df.getName(), name, duration, df.length(), df.getUri().toString(), null));
    }
            ...
            if (f1.isDirectory()) {
                ...
                if (SupportedFilesHelper.isText(f1)) {
                    ...
                    audioFileInfoArrayList
                            .add(new AudioFileInfo(f1.getName(), displayPath, duration, size, f1.getUri().toString(), null));
                    fullFolderSize += size;
