...
public class RadioFavoritesStore {
    ...
    private static final Type TYPE_MAP   = new TypeToken<Map<String, RadioFavoriteItem>>(){}.getType();
    private static final Type TYPE_ORDER = new TypeToken<List<String>>(){}.getType();
    ...
    public synchronized List<RadioFavoriteItem> getAll() {
        ...
        for (Map.Entry<String, RadioFavoriteItem> e : map.entrySet()) {
            if (!order.contains(e.getKey())) out.add(e.getValue());
        }
        return out;
    }
