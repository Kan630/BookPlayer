...
public final class AudioProber {
    ...
    @Nullable
    private static AudioInfo tryWithMediaExtractorOnFile(File file, Uri originalUri, String display,
            String sourceHint) {
        if (file == null || !file.exists() || file.length() <= 0)
            return null;
        MediaExtractor ex = new MediaExtractor();
    ...
    private static AudioInfo buildInfoFromRetriever(@Nullable Context context,
        ...
        // extractor.setDataSource(filePath); MediaFormat format =
