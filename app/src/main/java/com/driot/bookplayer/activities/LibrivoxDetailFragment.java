...
@AndroidEntryPoint
public class LibrivoxDetailFragment extends LoggingFragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        return inflater.inflate(R.layout.activity_librivox_detail, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        titleView = view.findViewById(R.id.textDetailTitle);
        infoView = view.findViewById(R.id.textDetailInfo);
        synopsisView = view.findViewById(R.id.textDetailSynopsis);
        coverView = view.findViewById(R.id.imageDetailCover);
        bGet = view.findViewById(R.id.bGetLibrivoxBook);
        tvLinkArchive = view.findViewById(R.id.tvLinkArchive);
        tvLinkLibrivox = view.findViewById(R.id.tvLinkLibrivox);
        tvDownloadLink = view.findViewById(R.id.tvDownloadLink);
        llWaitMessage = view.findViewById(R.id.ll_wait_message);
        tvWaitMain = view.findViewById(R.id.tvWaitMain);
        tvWaitDetail = view.findViewById(R.id.tvWaitDetail);

        // Init ViewModel
        viewModel = new ViewModelProvider(this).get(LibrivoxDetailViewModel.class);
        Bundle args = getArguments();
        viewModel.identifier = args != null ? args.getString("identifier") : null;
        viewModel.title = args != null ? args.getString("title") : null;

        titleView.setText(viewModel.title);
        infoView.setText(getString(R.string.loading_details));

        // Load cached image if any
        File localImage = ImageHelper.getLibrivoxImageFile(requireContext(), viewModel.identifier);
        ...
        // Observers
        viewModel.metadata.observe(getViewLifecycleOwner(), this::showMetadata);

        // Observe existing folder
        viewModel.existingFolder.observe(getViewLifecycleOwner(), folder -> {
            if (folder != null) {
        ...
        networkMonitor = new NetworkStatusMonitor(requireContext(), hasInternet -> {
    ...
    private void updateUiForNetworkState(boolean hasInternet) {
        if (!hasInternet) {
            ...
            requireView().findViewById(R.id.ll_links).setVisibility(View.GONE);
        } else {
            ...
            if (viewModel.metadata.getValue() == null) {
                infoView.setText(getString(R.string.loading_details));
                fetchMetadata();
                if (futureCoverPic == null && !ImageHelper.getLibrivoxImageFile(requireContext(), viewModel.identifier).exists()) {
                    fetchFallbackImage();
                }
            } else {
            ...
            if (viewModel.download_link.getValue() == null) {
                tvDownloadLink.setText(getString(R.string.librivox_zip_mp3_checking));
    ...
    private void setupDownloadLinkObserver() {
        viewModel.download_link.observe(getViewLifecycleOwner(), download_link -> {
    ...
    private void showMetadata(ItemMetadata metadata) {
        ...
        if (librivoxUrl != null) {
            ...
            requireView().findViewById(R.id.ll_link_librivox).setVisibility(View.VISIBLE);
        } else {
            requireView().findViewById(R.id.ll_link_librivox).setVisibility(View.GONE);
        }
    }
                ...
                requireActivity().runOnUiThread(() -> {
                    ...
                    if (Option.getNetworkPolicyManualDownload()
                        ...
                        if (Option.getNetworkPolicyManualDownload()
                                ...
                                && !NetworkHelper.isRoaming(requireContext())) {
                            pendingDownloadUrl = url;
                            pendingDownloadPath = futurePath;
                            MsgBox.ask(this,
    ...
    private void proceedWithDownload(String url, String futurePath) {
        ...
        state.originalFile = SupportedFilesHelper.getFileName(requireContext(), Uri.parse(url));
        ...
        BookLoadingWorkLauncher.launch(requireContext(), state, false);

        FirebaseAnalyticsHelper.tellLibrivoxDownload(state.title);
        // Original Activity behavior: finish() this screen only, revealing the results/favorites
        // list underneath so the user can keep browsing while this download runs in background.
        // Now that this is a Fragment inside AddBookHostActivity's shared graph, the equivalent
        // is popping back one level, NOT finishing the whole host Activity.
