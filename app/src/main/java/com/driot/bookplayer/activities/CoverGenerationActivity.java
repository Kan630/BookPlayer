...
// Use passed initials if any, else derive from title
        ...
        edtInitials.setFilters(new InputFilter[]{ new InputFilter.LengthFilter(5) });

        switchRounded.setChecked(rounded);

        int r = Color.red(defaultColor), g = Color.green(defaultColor), b = Color.blue(defaultColor);
        setupNumberBox(edtR); setupNumberBox(edtG); setupNumberBox(edtB);
        edtR.setText(String.valueOf(r)); seekR.setMax(255); seekR.setProgress(r);
        edtG.setText(String.valueOf(g)); seekG.setMax(255); seekG.setProgress(g);
        edtB.setText(String.valueOf(b)); seekB.setMax(255); seekB.setProgress(b);

        edtInitials.addTextChangedListener(watcher(this::updatePreview));
        switchRounded.setOnCheckedChangeListener((v, isChecked) -> { rounded = isChecked; updatePreview(); });

        hookSeekToBox(seekR, edtR); hookSeekToBox(seekG, edtG); hookSeekToBox(seekB, edtB);
        hookBoxToSeek(edtR, seekR); hookBoxToSeek(edtG, seekG); hookBoxToSeek(edtB, seekB);
    ...
    private void hookSeekToBox(SeekBar seek, EditText box) {
        seek.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            @Override public void onProgressChanged(SeekBar sb, int p, boolean fromUser) {
                if (!fromUser || updatingFromText) return;
    ...
    private String getDefaultInitials(String title) {
        if (title == null) return "AB";
        String t = title.trim();
        if (t.isEmpty()) return "AB";
        ...
        if (res.isEmpty()) res = t.substring(0, Math.min(2, t.length())).toUpperCase(Locale.US);
        return res;
    }
