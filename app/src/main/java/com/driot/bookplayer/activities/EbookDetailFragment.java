...
@AndroidEntryPoint
public class EbookDetailFragment extends LoggingFragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        return inflater.inflate(R.layout.activity_ebook_detail, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        tvTitle = view.findViewById(R.id.textDetailTitle);
        tvAuthors = view.findViewById(R.id.textDetailAuthors);
        tvInfo = view.findViewById(R.id.textDetailInfo);
        tvGutenbergLink = view.findViewById(R.id.tvGutenbergLink);
        tvDescription = view.findViewById(R.id.tvDescription);
        tvStatus = view.findViewById(R.id.tvStatus);
        bGet = view.findViewById(R.id.bGet);

        // --- Read arguments ---
        Bundle args = getArguments();
        gutendexId = args != null ? args.getInt("gutendex_id", 0) : 0;
        title = args != null ? args.getString("title") : null;
        authors = args != null ? args.getString("authors") : null;
        language = args != null ? args.getString("language") : null;
        downloads = args != null ? args.getInt("downloads", 0) : 0;
        coverUrl = args != null ? args.getString("cover_url") : null;
        epubUrl = args != null ? args.getString("epub_url") : null;

        if (epubUrl == null || epubUrl.isEmpty() || gutendexId == 0) {
            myLogE("EbookDetailFragment: invalid arguments, popping back.");
        ...
        java.io.File persistedCover = ImageHelper.getGutendexImageFile(requireContext(), gutendexId);
        ...
        viewModel.getBookData().observe(getViewLifecycleOwner(), this::onBookRefreshed);
    ...
    private void onBookRefreshed(GutendexBook book) {
        ...
        java.io.File imageFile = ImageHelper.getGutendexImageFile(requireContext(), gutendexId);
        String savedUrl = ImageHelper.getGutendexSavedCoverUrl(requireContext(), gutendexId);
    ...
    private void checkThenDownload(String url) {
        ...
        String futurePath = getUnzipFolder(requireContext()).getAbsolutePath()
    ...
    private void proceedWithDownload(String url, String futurePath) {
        ...
        state.originalFile = SupportedFilesHelper.getFileName(requireContext(), epubUri);
        state.sourceLocation = SOURCE_LOCATION_EBOOK_GUTENDEX;
        state.fileExtension = "epub";

        // Compute originalHash for the EPUB URL
        try {
            String hash = HashWorker.computeHashFromUri(requireContext(), epubUri);
        ...
        java.io.File coverFile = ImageHelper.getGutendexImageFile(requireContext(), gutendexId);
        ...
        // Original Activity behavior: finish() this screen only, revealing the results list
        // underneath. Now that this is a Fragment inside AddBookHostActivity's shared graph, pop
        // back one level instead of finishing the whole host Activity.
